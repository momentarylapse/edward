...
namespace polymesh {
	...
	public:
		...
		GeometryCylinder(Array<vec3> &pos, float radius, int rings, int edges, End end_mode)
		{
			Interpolator<float> inter_r(Interpolator<float>::TYPE_CUBIC_SPLINE_NOTANG);
			inter_r.add(radius);
			inter_r.add(radius);
		...
		GeometryCylinder(const vec3 &pos1, const vec3 &pos2, float radius, int rings, int edges, End end_mode)
		{
			Interpolator<float> inter_r(Interpolator<float>::TYPE_CUBIC_SPLINE_NOTANG);
			inter_r.add(radius);
			inter_r.add(radius);
		...
		void buildFromPath(Interpolator<vec3> &inter, Interpolator<float> &inter_r, int rings, int edges, End end_mode)
		{
			if (end_mode == End::LOOP){
				inter.close();
				inter_r.close();
			}
			Array<vec3> sv;
			vec3 r_last = v_0;
			int rings_vertex = (end_mode == End::LOOP) ? rings : (rings + 1);
			mat4 frame0, frame1;

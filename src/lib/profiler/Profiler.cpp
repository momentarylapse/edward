...
namespace profiler {
	...
	static constexpr int max_history_length = 1024;
	...
	struct State {
		...
		Array<ChannelStats> digest_report(const FrameTimingData& td) const {
	...
	void init_external(void* x_state) {
		state = reinterpret_cast<State*>(x_state);
	}
	...
	int create_channel(const string &name, int parent) {
		...
		state->channels.add({name, true, parent});
	...
	void begin(int channel) {
		auto now = std::chrono::high_resolution_clock::now();
		state->current_frame_timing.cpu0.add({channel, std::chrono::duration<float, std::chrono::seconds::period>(now - state->frame_start).count()});
	}

	void end(int channel) {
		auto now = std::chrono::high_resolution_clock::now();
		state->current_frame_timing.cpu0.add({channel | (int)0x80000000, std::chrono::duration<float, std::chrono::seconds::period>(now - state->frame_start).count()});
	}
	...
	bool just_cleared() {
		return state->just_cleared;
	}

	float temp_frame_time() {
		return state->temp_frame_time;
	}
	float avg_frame_time() {
		return state->avg_frame_time;
	}
	...
	FrameTimingData& current_frame_timing() {
		return state->current_frame_timing;
	}
	FrameTimingData& previous_frame_timing() {
		return state->previous_frame_timing;
	}
	FrameHistory& history() {

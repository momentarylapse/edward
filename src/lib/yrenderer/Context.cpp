...
base::result<Material*> Context::load_material(const Path &filename) const {
...
base::result<shared<ygfx::Texture>> Context::load_texture(const Path& filename) const {
	return texture_manager->load_texture(filename);
}

base::result<shared<ygfx::Shader>> Context::load_shader(const Path& path) const {
	return shader_manager->load_shader(path);
}
base::result<shared<ygfx::Shader>> Context::create_shader(const string &source) const {

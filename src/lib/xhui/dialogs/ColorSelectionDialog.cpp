...
public:
	...
	void on_left_button_down(const vec2& m) override {
		hue = clamp((m.y - area.y1) / area.height(), 0.0f, 1.0f);
		request_redraw();
		if (callback)
			callback(hue);
	...
	void _draw(Painter* p) override {
		...
		for (int i=0; i<N; i++) {
			...
			p->draw_rect({area.x1, area.x2, area.y1 + (float)i / (float)N * h, area.y1 + (float)(i + 1) / (float)N * h});
			p->set_color(color::from_hsb(color::from_hsb((float)i / (float)N, 1, 1, 1).hue(), 1, 1, 1));
			p->draw_rect({area.x1 + w/2, area.x2, area.y1 + (float)i / (float)N * h, area.y1 + (float)(i + 1) / (float)N * h});
		}

		p->set_color(Black);
		p->draw_circle(area.p00() + vec2(w/2, hue * h), 6);
		p->set_color(White);
		p->draw_circle(area.p00() + vec2(w/2, hue * h), 4);
	}
};
...
public:
	...
	void on_left_button_down(const vec2& m) override {
		saturation = clamp((m.x - area.x1) / area.width(), 0.0f, 1.0f);
		brightness = clamp(1 - (m.y - area.y1) / area.height(), 0.0f, 1.0f);
		request_redraw();
		emit_event(event_id::Changed, true);
		if (callback)
	...
	void _draw(Painter* p) override {
		...
		for (int i=0; i<N; i++) {
			for (int j=0; j<N; j++) {
				...
				p->draw_rect({area.x1 + (float)i / (float)N * w, area.x1 + (float)(i + 1) / (float)N * w, area.y1 + (float)j / (float)N * h, area.y1 + (float)(j + 1) / (float)N * h});
			}
		}

		p->set_color(Black);
		p->draw_circle(area.p00() + vec2(saturation * w, (1 - brightness) * h), 6);
		p->set_color(White);
		p->draw_circle(area.p00() + vec2(saturation * w, (1 - brightness) * h), 4);
	}
};
...
public:
	...
	void on_left_button_down(const vec2& m) override {
		alpha = clamp((m.x - area.x1) / area.width(), 0.0f, 1.0f);
		request_redraw();
		if (callback)
			callback(alpha);
		emit_event(event_id::Changed, true);
	...
	void _draw(Painter* p) override {
		...
		draw_checkerboard(p, area);

		const int N = 100;
		for (int i=0; i<N; i++) {
			p->set_color(col.with_alpha((float)i / (float)N));
			p->draw_rect({area.x1 + (float)i / (float)N * w, area.x1 + (float)(i + 1) / (float)N * w, area.y1, area.y2});
		}

		p->set_color(Black);
		p->draw_circle(area.p00() + vec2(alpha * w, h/2), 6);
		p->set_color(White);
		p->draw_circle(area.p00() + vec2(alpha * w, h/2), 4);
	}
};
...
ColorSelectionDialog::ColorSelectionDialog(Panel* parent, const string& title, const color& col, const Array<string>& params) : Dialog(title, 600, 600, parent) {
	...
	indicator_original->size_mode_x = SizeMode::Fill;
	indicator_original->size_mode_y = SizeMode::Fill;
	...
	indicator->size_mode_x = SizeMode::Fill;
	indicator->size_mode_y = SizeMode::Fill;
